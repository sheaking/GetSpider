...
class Loggers(object):
    ...
    def __init__(self, filename='{uid}.log'.format(uid=uuid.uuid4()), level='info', log_dir='log',
        ...
        self.logger = logging.getLogger(filename)  # 得到logger实例对象

        abspath = os.path.dirname(os.path.abspath(__file__))  # 得到当前py文件目录

        self.directory = os.path.join(abspath, log_dir)  # 设置日志目录
